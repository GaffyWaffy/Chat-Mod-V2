...
public final class ChatRouter {
    ...
    private String copyFlashWindowId;
    private int copyFlashTick = -1000;
    ...
    public boolean dispatch(Text text) {
        ...
        ReceivedMessage message = new ReceivedMessage(text, plain, ticks, System.currentTimeMillis());
    ...
    private void route(ReceivedMessage message, boolean allowSound) {
        ...
        for (ChatWindow window : cfg.windows) {
            for (int i = 0; i < window.tabs.size(); i++) {
                ...
                tab.addMessage(message, tab.highlightFor(message.plain()), window, client.textRenderer);
    ...
    public void render(DrawContext context) {
        ...
        for (ChatWindow window : cfg.windows) {
            ...
            ChatWindowRenderer.render(context, window, chatOpen, false, lastMouseX, lastMouseY);
        }
    }
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        ...
        String toCopy = ChatWindowRenderer.copyTargetAt(window, lx, ly, client.textRenderer);
        if (toCopy != null) {
            client.keyboard.setClipboard(toCopy);
            copyFlashWindowId = window.id;
            copyFlashTick = ticks;
            return true;
        }
    ...
    /** True for ~1.5s after a message was copied from this window. */
    public boolean copyFlash(ChatWindow window) {
        return window.id.equals(copyFlashWindowId) && ticks - copyFlashTick < 30;
    }
