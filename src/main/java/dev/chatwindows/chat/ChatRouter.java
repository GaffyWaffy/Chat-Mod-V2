...
public final class ChatRouter {
    ...
    private void route(ReceivedMessage message, boolean allowSound) {
        ...
        for (ChatWindow window : cfg.windows) {
            for (int i = 0; i < window.tabs.size(); i++) {
                ...
                HighlightRule rule = tab.matchingHighlight(message.plain());
                int highlight = rule == null ? 0 : rule.argb();
                tab.addMessage(message, highlight, window.maxMessages, client.textRenderer);

                if (i != window.activeTab) tab.unread = true;

                if (allowSound && !playedSound && rule != null && rule.playSound && client.player != null) {
    ...
    public Style styleAt(double mouseX, double mouseY) {
        ...
        int sw = client.getWindow().getScaledWidth();
        int sh = client.getWindow().getScaledHeight();
        return ChatWindowRenderer.styleAt(window,
                ChatWindowRenderer.localX(window, mouseX, sw),
                ChatWindowRenderer.localY(window, mouseY, sh),
                client.textRenderer);
    }
    ...
    public boolean mouseScrolled(double mouseX, double mouseY, double amount) {
        ...
        int step = net.minecraft.client.gui.screen.Screen.hasControlDown() ? 1 : 3;
