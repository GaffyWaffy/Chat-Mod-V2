...
public final class ChatWindowRenderer {
    ...
    /** The click/hover style under the cursor, or null. */
    public static Style styleAt(ChatWindow window, double localX, double localY, TextRenderer textRenderer) {
        ChatTab tab = window.activeTab();
        if (tab == null) return null;
        if (window.showTabBar && localY < TAB_HEIGHT) return null;

        for (Line line : layout(window, tab, textRenderer)) {
            if (localY >= line.localY() - 1 && localY < line.localY() + window.lineSpacing - 1) {
                int textX = (int) (localX - PADDING - 1);
                if (textX < 0) return null;
                return textRenderer.getTextHandler().getStyleAt(line.text(), textX);
            }
        }
        return null;
    }
