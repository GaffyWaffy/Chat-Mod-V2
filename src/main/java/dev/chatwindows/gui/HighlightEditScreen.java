...
public class HighlightEditScreen extends BaseSettingsScreen {
    ...
    protected void init() {
        ...
        addDrawableChild(CyclingButtonWidget.<MatchType>builder(m -> Text.literal(m.display()))
                .values(MatchType.values())
                .initially(rule.match)
        ...
        addDrawableChild(CyclingButtonWidget.onOffBuilder(rule.enabled)
                .build(cx - 158, y, 155, 20, Text.literal("Enabled"), (b, v) -> rule.enabled = v));
        addDrawableChild(CyclingButtonWidget.onOffBuilder(rule.playSound)
                .build(cx + 1, y, 155, 20, Text.literal("Play sound"), (b, v) -> rule.playSound = v));
        y += 30;
